...
DllExportFwd  int	win32_vfprintf(FILE *pf, const char *format, va_list arg);
DllExportFwd  int	win32_vprintf(const char *format, va_list arg);
DllExportFwd  size_t	win32_fread(void *buf, size_t size, size_t count, FILE *pf);
DllExportFwd  size_t	win32_fwrite(const void *buf, size_t size, size_t count, FILE *pf);
...
DllExportFwd  int	win32_fputs(const char *s,FILE *pf);
DllExportFwd  int	win32_fputc(int c,FILE *pf);
DllExportFwd  int	win32_ungetc(int c,FILE *pf);
DllExportFwd  int	win32_getc(FILE *pf);
DllExportFwd  int	win32_fileno(FILE *pf);
DllExportFwd  void	win32_clearerr(FILE *pf);
DllExportFwd  int	win32_fflush(FILE *pf);
DllExport  Off_t	win32_ftell(FILE *pf);
DllExport  int		win32_fseek(FILE *pf,Off_t offset,int origin);
DllExportFwd  int	win32_fgetpos(FILE *pf,fpos_t *p);
DllExportFwd  int	win32_fsetpos(FILE *pf,const fpos_t *p);
DllExportFwd  void	win32_rewind(FILE *pf);
DllExport  int		win32_tmpfd(void);
DllExport  FILE*	win32_tmpfile(void);
DllExportFwd  void	win32_abort(void);
DllExport  int  	win32_fstat(int fd,Stat_t *sbufptr);
DllExport  int  	win32_stat(const char *name,Stat_t *sbufptr);
DllExportFwd  int	win32_pipe( int *phandles, unsigned int psize, int textmode );
...
DllExportFwd  int	win32_setmode( int fd, int mode);
...
DllExportFwd  int	win32_dup( int fd);
DllExportFwd  int	win32_dup2(int h1, int h2);
DllExport  int		win32_open(const char *path, int oflag,...);
DllExport  int		win32_close(int fd);
DllExportFwd  int	win32_eof(int fd);
DllExport  int		win32_isatty(int fd);
DllExportFwd  int	win32_read(int fd, void *buf, unsigned int cnt);
DllExportFwd  int		win32_write(int fd, const void *buf, unsigned int cnt);
...
DllExportFwd  void	win32_perror(const char *str);
DllExportFwd  void	win32_setbuf(FILE *pf, char *buf);
DllExportFwd  int	win32_setvbuf(FILE *pf, char *buf, int type, size_t size);
DllExportFwd  int	win32_flushall(void);
DllExportFwd  int	win32_fcloseall(void);
DllExportFwd  char*	win32_fgets(char *s, int n, FILE *pf);
DllExportFwd  char*	win32_gets(char *s);
DllExportFwd  int	win32_fgetc(FILE *pf);
DllExportFwd  int	win32_putc(int c, FILE *pf);
DllExportFwd  int	win32_puts(const char *s);
DllExportFwd  int	win32_getchar(void);
DllExportFwd  int	win32_putchar(int c);
DllExportFwd  void*	win32_malloc(size_t size);
DllExportFwd  void*	win32_calloc(size_t numitems, size_t size);
DllExportFwd  void*	win32_realloc(void *block, size_t size);
DllExportFwd  void	win32_free(void *block);

DllExportFwd  int	win32_open_osfhandle(intptr_t handle, int flags);
DllExportFwd  intptr_t	win32_get_osfhandle(int fd);
/* for testing DLL export forwarding */
DllExportFwd  intptr_t	win32_get_heap_handle(void);
DllExport  ioinfo*	win32_get_pioinfo(void);
DllExportFwd  ioinfo*	win32_pioinfo[];
